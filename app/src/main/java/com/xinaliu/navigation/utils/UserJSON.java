...
public class UserJSON {
	...
	public static <T> T parseObject(Class<T> clazz,String data) {
		try {
			if (data == null)return instantiationClass(clazz);
			return parseObject(clazz,new JSONObject(data));
		} catch (JSONException e) {
			e.printStackTrace();
		}
		return instantiationClass(clazz);
	}
	/***
	 * json 解析成实体
	 * @param cls
	 ...
	 */
	public static  <T> T parseObject(Class<T> clazz, JSONObject jsonObject) {
...
//					fieldsAndValues.put(ff.getName(), value);
	...
	public static <T> T getT(Class<T> clazz, JSONObject jsonObject,String key) {
		try {
			...
			else if (Date.class == clazz || java.sql.Date.class == clazz) {
	 ...
	 * @return
	 */
	public static <T> T parseEntity(Class<T> clazz, JSONObject jsonObject,boolean is) {
									...
									try {
										if(object instanceof JSONArray){
											...
											if(list != null){
												...
												field.set(obj, parsUser(forName, jsonArray, true));
											}
